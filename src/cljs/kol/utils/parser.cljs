(ns kol.utils.parser
  (:require
   ...
   [kol.defs :refer [Block]]))

(declare sexpr->blk)

(defn create-block-def [sexpr idx lvl]
  (Block. sexpr idx lvl))
...
(defn sexpr->blk
  ...
  (if (list? xs)
    (let [f (first xs)
          args (rest xs)]
      (case f
        defn (defn->block xs idx lvl)
        fn (fn->block xs idx lvl)
         ;; default
