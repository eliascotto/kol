...
(add||=[]).push(pos);add&&(match.set=add.concat(match.set).sort((a,b)=>a.cmp(b)))}}timing&&console.log(`Merge identical pass ${pass}${didMerge?"":", done"} (${((Date.now()-t0)/1E3).toFixed(2)}s)`);if(!didMerge)return states;for(let state of newStates)state.defaultReduce||(state.actions=state.actions.map(a=>a.map(mapping,newStates)),state.goto=state.goto.map(a=>a.map(mapping,newStates)));for(states=0;states<newStates.length;states++)newStates[states].id=states;states=newStates}}function encode(value,
