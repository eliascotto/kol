...
left&&(cur.scrollLeft=left)}}function textRange(node,from,to=from){let range=scratchRange||=document.createRange();range.setEnd(node,to);range.setStart(node,from);return range}function dispatchKey(elt,name,code){code={key:name,code:name,keyCode:code,which:code,cancelable:!0};name=new KeyboardEvent("keydown",code);name.synthetic=!0;elt.dispatchEvent(name);code=new KeyboardEvent("keyup",code);code.synthetic=!0;elt.dispatchEvent(code);return name.defaultPrevented||code.defaultPrevented}function rm(dom){let next=
dom.nextSibling;dom.parentNode.removeChild(dom);return next}function textCoords(text,pos,side){let length=text.nodeValue.length;pos>length&&(pos=length);let from=pos,to=pos,flatten=0;0==pos&&0>side||pos==length&&0<=side?browser.chrome||browser.gecko||(pos?(from--,flatten=1):(to++,flatten=-1)):0>side?from--:to++;text=textRange(text,from,to).getClientRects();if(!text.length)return Rect0;side=text[(flatten?0>flatten:0<=side)?0:text.length-1];browser.safari&&!flatten&&0==side.width&&(side=Array.prototype.find.call(text,
r=>r.width)||side);return flatten?flattenRect(side,0>flatten):side}function mergeInlineChildren(parent,from,to,elts,openStart,openEnd){let cur=parent.childCursor(),{i:toI,off:toOff}=cur.findPos(to,1),{i:fromI,off:fromOff}=cur.findPos(from,-1);from-=to;for(var view of elts)from+=view.length;parent.length+=from;({children:view}=parent);if(fromI==toI&&fromOff){from=view[fromI];if(1==elts.length&&from.merge(fromOff,toOff,elts[0],openStart,openEnd))return;if(0==elts.length){from.merge(fromOff,toOff,null,
openStart,openEnd);return}from=from.slice(toOff);from.merge(0,0,elts[elts.length-1],0,openEnd)?elts[elts.length-1]=from:elts.push(from);toI++;openEnd=toOff=0}toOff?(from=view[toI],elts.length&&from.merge(0,toOff,elts[elts.length-1],0,openEnd)?(elts.pop(),openEnd=elts.length?0:openStart):from.merge(0,toOff,null,0,0)):toI<view.length&&elts.length&&view[toI].merge(0,0,elts[elts.length-1],0,openEnd)&&(elts.pop(),openEnd=elts.length?0:openStart);fromOff?(from=view[fromI],elts.length&&from.merge(fromOff,
...
textRange(node,i,i+1).getClientRects();for(let j=0;j<rects.length;j++){let rect=rects[j];if(rect.top==rect.bottom)continue;generalSide||=x-rect.left;let dy=(rect.top>y?rect.top-y:y-rect.bottom)-1;if(rect.left-1<=x&&rect.right+1>=x&&dy<closestDY){closestDY=closestOffset=x>=(rect.left+rect.right)/2;(browser.chrome||browser.gecko)&&textRange(node,i).getBoundingClientRect().left==rect.right&&(closestDY=!closestOffset);if(0>=dy)return{node,offset:i+(closestDY?1:0)};closestOffset=i+(closestDY?1:0);closestDY=
dy}}}return{node,offset:-1<closestOffset?closestOffset:0<generalSide?node.nodeValue.length:0}}function posAtCoords(view,{x,y},precise,bias=-1){var content=view.contentDOM.getBoundingClientRect();let halfLine=view.defaultLineHeight/2;for(let bounced=!1;;){var block=view.blockAtHeight(y,content.top);if(block.top>y||block.bottom<y){bias=block.top>y?-1:1;y=Math.min(block.bottom-halfLine,Math.max(block.top+halfLine,y));if(bounced)return precise?null:0;bounced=!0}if(block.type==exports.BlockType.Text)break;
...
view.contentDOM;let contentRect=content.getBoundingClientRect(),base=getBase(view);content=window.getComputedStyle(content.firstChild);let leftSide=contentRect.left+parseInt(content.paddingLeft),rightSide=contentRect.right-parseInt(content.paddingRight);content=blockAt(view,from$jscomp$1);let endBlock=blockAt(view,to$jscomp$1),visualStart=content.type==exports.BlockType.Text?content:null,visualEnd=endBlock.type==exports.BlockType.Text?endBlock:null;view.lineWrapping&&(visualStart&&=wrappedLine(view,
from$jscomp$1,visualStart),visualEnd&&=wrappedLine(view,to$jscomp$1,visualEnd));if(visualStart&&visualEnd&&visualStart.from==visualEnd.from)return pieces(drawForLine(range.from,range.to,visualStart));from$jscomp$1=visualStart?drawForLine(range.from,null,visualStart):drawForWidget(content,!1);range=visualEnd?drawForLine(null,range.to,visualEnd):drawForWidget(endBlock,!0);to$jscomp$1=[];(visualStart||content).to<(visualEnd||endBlock).from-1?to$jscomp$1.push(piece(leftSide,from$jscomp$1.bottom,rightSide,
range.top)):from$jscomp$1.bottom<range.top&&blockAt(view,(from$jscomp$1.bottom+range.top)/2).type==exports.BlockType.Text&&(from$jscomp$1.bottom=range.top=(from$jscomp$1.bottom+range.top)/2);return pieces(from$jscomp$1).concat(to$jscomp$1).concat(pieces(range))}function measureCursor(view,cursor,primary){cursor=view.coordsAtPos(cursor.head,cursor.assoc||1);if(!cursor)return null;view=getBase(view);return new Piece(cursor.left-view.left,cursor.top-view.top,-1,cursor.bottom-cursor.top,primary?"cm-cursor cm-cursor-primary":
"cm-cursor cm-cursor-secondary")}function iterMatches(doc,re,from,to,f){re.lastIndex=0;for(let cursor=doc.iterRange(from,to),pos=from,m;!cursor.next().done;pos+=cursor.value.length)if(!cursor.lineBreak)for(;m=re.exec(cursor.value);)f(pos+m.index,pos+m.index+m[0].length,m)}function specialCharPlugin(){return _plugin||=ViewPlugin.fromClass(class{constructor(view){this.view=view;this.decorations=Decoration.none;this.decorationCache=Object.create(null);this.decorator=this.makeDecorator(view.state.facet(specialCharConfig));
this.decorations=this.decorator.createDeco(view)}makeDecorator(conf){return new MatchDecorator({regexp:conf.specialChars,decoration:(m,view,pos)=>{let {doc}=view.state;m=text$jscomp$0.codePointAt(m[0],0);return 9==m?(m=doc.lineAt(pos),view=view.state.tabSize,pos=text$jscomp$0.countColumn(doc.sliceString(m.from,pos),0,view),Decoration.replace({widget:new TabWidget((view-pos%view)*this.view.defaultCharacterWidth)})):this.decorationCache[m]||(this.decorationCache[m]=Decoration.replace({widget:new SpecialCharWidget(conf,
m)}))},boundary:conf.replaceTabs?void 0:/[^]/})}update(update){let conf=update.state.facet(specialCharConfig);update.startState.facet(specialCharConfig)!=conf?(this.decorator=this.makeDecorator(conf),this.decorations=this.decorator.createDeco(update.view)):this.decorations=this.decorator.updateDeco(update,this.decorations)}},{decorations:v=>v.decorations})}Object.defineProperty(exports,"__esModule",{value:!0});var state$jscomp$0=require("module$node_modules$$codemirror$state$dist$index_cjs"),styleMod=
require("module$node_modules$style_mod$dist$style_mod_cjs"),rangeset=require("module$node_modules$$codemirror$rangeset$dist$index_cjs"),text$jscomp$0=require("module$node_modules$$codemirror$text$dist$index_cjs"),w3cKeyname=require("module$node_modules$w3c_keyname$index_cjs");const Rect0={left:0,right:0,top:0,bottom:0};class DOMSelection{constructor(){this.anchorNode=null;this.anchorOffset=0;this.focusNode=null;this.focusOffset=0}eq(domSel){return this.anchorNode==domSel.anchorNode&&this.anchorOffset==
...
bottom:pos.bottom+mBottom};pos=side.ownerDocument.defaultView;for(side=side.parentNode;side;)if(1==side.nodeType){if(mLeft=side==document.body){mBottom=0;var JSCompiler_object_inline_right_525=pos.innerWidth;var JSCompiler_object_inline_top_526=0;var JSCompiler_object_inline_bottom_527=pos.innerHeight}else{if(side.scrollHeight<=side.clientHeight&&side.scrollWidth<=side.clientWidth){side=side.parentNode;continue}mRight=side.getBoundingClientRect();mBottom=mRight.left;JSCompiler_object_inline_right_525=
mRight.left+side.clientWidth;JSCompiler_object_inline_top_526=mRight.top;JSCompiler_object_inline_bottom_527=mRight.top+side.clientHeight}mTop=mRight=0;margins.top<JSCompiler_object_inline_top_526?mTop=-(JSCompiler_object_inline_top_526-margins.top+5):margins.bottom>JSCompiler_object_inline_bottom_527&&(mTop=margins.bottom-JSCompiler_object_inline_bottom_527+5);margins.left<mBottom?mRight=-(mBottom-margins.left+5):margins.right>JSCompiler_object_inline_right_525&&(mRight=margins.right-JSCompiler_object_inline_right_525+
